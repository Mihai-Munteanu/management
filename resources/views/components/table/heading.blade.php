...
<th
    {{ $attributes->merge(['class' => 'text-left px-4 py-3 bg-cool-gray-50'])->only('class') }}
>
    @unless ($sortable)
        <span class="text-left text-sm leading-4 text-gray-700 font-bold tracking-wider">{{ $slot }}</span>
    @else
        <button {{ $attributes->except('class') }} class="flex items-center space-x-1 text-left text-sm leading-4 text-gray-700 font-bold tracking-wider group focus:outline-none">
            <span>{{ $slot }}</span>
